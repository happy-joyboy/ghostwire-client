...
// Allows the UI to change the port dynamically
export const setHardwarePort = (port: number) => {
  currentPort = port;
  console.log(`[NETWORK] Hardware bridge redirected to port ${currentPort}`);
};

// Helper to build the correct URL dynamically based on the active user
...
export const getPublicKey = async (): Promise<string> => {
  ...
  const res = await fetchWithTimeout(`${getBaseUrl()}/handshake`, {
    method: "GET",
  });
